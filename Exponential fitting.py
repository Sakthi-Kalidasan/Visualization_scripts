...
def logarithmic_func(x, a, b):
    return a * np.log((0.75*1.8)/(5.1*x)) + b

# Load the CSV file
data = pd.read_csv(r't_settl_75per.csv')

# Assuming the CSV has columns 'x' and 'y'
x_data = data['t_sett_75per X']
y_data = data['t_sett_75per Y']
...
equation_text = f't_set = {a*1e9:.2f}n * log((0.75*1.8)/(5.1*vin)) + {b*1e9:.2f}n'
